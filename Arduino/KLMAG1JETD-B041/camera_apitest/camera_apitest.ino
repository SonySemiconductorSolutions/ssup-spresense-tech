...
RtcTime time_now;
RtcTime time_end;
const int FRAMES_NUM = 200; /* 120FPS * 5 seconds */
  ...
  if (fps_flag)
    {
      ...
      if (img.isAvailable())
        {
          if (frame_count%(FRAMES_NUM/4) == 0)
            {
              Serial.print(frame_count);
              Serial.print("/");
              Serial.print(FRAMES_NUM);
              Serial.println(" frames completed.");
            }
          frame_count++;
        }
...
void menuTop()
{
  ...
  Serial.println("M    : Calculate FPS");
  Serial.println();
