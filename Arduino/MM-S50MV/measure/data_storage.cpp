...
void DataStorageClass::SaveData(int32_t onedimensional_distance, uint16_t onedimensional_lightquantity,
  ...
  OpenFile(false);  
  snprintf(data_time, sizeof(data_time), "Time：%s%04d/%02d/%02d/%02d:%02d:%02d%s\n", CSV_FORMAT, rtc.year(), rtc.month(),rtc.day(),rtc.hour(), rtc.minute(),rtc.second(), CSV_FORMAT); 

  char oned_distance[64] = {0};
  snprintf(oned_distance, sizeof(oned_distance), "1D_Distance(mm)：%s%d%s\n", CSV_FORMAT, onedimensional_distance, CSV_FORMAT);
  char oned_lightquantity[64];
  snprintf(oned_lightquantity, sizeof(oned_lightquantity), "1D_LightQuantity：%s%d%s\n", CSV_FORMAT, onedimensional_lightquantity, CSV_FORMAT);
  char merge_3d_distance[512] = {0};
  char merge_3d_lightquantity[512] = {0};
  char format3d_distance[32] = {0};
  char format3d_lightquantity[32] = {0};
  
  snprintf(format3d_distance, sizeof(format3d_distance), "3D_Distance(mm)：%s", CSV_FORMAT);
  strncat(merge_3d_distance,format3d_distance,sizeof(format3d_distance));
  
  for (int i = 0; i < 32; i++) {
    memset(format3d_distance, 0, 32);
    snprintf(format3d_distance, sizeof(format3d_distance), "%d%s", *(threedimensional_distance + i), CSV_FORMAT);
    strncat(merge_3d_distance,format3d_distance,sizeof(format3d_distance));
  }
  strncat(merge_3d_distance,"\n",sizeof("\n"));
  
  snprintf(format3d_lightquantity, sizeof(format3d_lightquantity), "3D_LightQuantity：%s", CSV_FORMAT);
  strncat(merge_3d_lightquantity,format3d_lightquantity,sizeof(format3d_lightquantity));
  
  for (int i = 0; i < 32; i++) {
    memset(format3d_lightquantity, 0, 32);
    snprintf(format3d_lightquantity, sizeof(format3d_lightquantity), "%d%s", *(threedimensional_lightquantity + i), CSV_FORMAT);
    strncat(merge_3d_lightquantity,format3d_lightquantity,sizeof(format3d_lightquantity));
  }
  strncat(merge_3d_lightquantity,"\n",sizeof("\n"));

  snprintf(data, sizeof(data), "%s%s%s%s%s\n", data_time, oned_distance, oned_lightquantity,merge_3d_distance,merge_3d_lightquantity);
  Serial.print(data);
  myFile.print(data);
  myFile.close();
}
